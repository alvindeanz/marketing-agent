...
function base() {
  return {
    title: 'T 搜索广告素材方向', period_label: '2026 年 8 月', oneline: '方向。',
    bignums: [{ k: '花了', v: '1', s: 's' }, { k: '最佳广告换来', v: '2', unit: '条', s: 'Free Quotation 那条' }, { k: '一条', v: '3', s: 's' }],
    fineprint: ['<b>询价</b>口径。'],
    lights: [{ name: 'louvre roof 组的广告', state_label: '换上新文案', cls: 'g' }],
    s1_desc: 'd', s2_desc: 'd',
    ...
    families: [{ name: 'louvre roof 组的广告一', cls: 'g', state_label: '保持', spent: 'a', got: 'b', next: 'c', evidence: 'e' }],
    negatives: ['清理段。'], findings_title: '三条新发现', findings: ['f'], confirm: ['首版。'],
...
const s2 = base(); s2.findings = ['这条广告 Ad Strength 低'];
assert.throws(() => validate(s2), /立场 2/);
// 立场 1：素材级不算单条询价成本
const s1 = base(); s1.negatives = ['素材级单条询价成本 5 纽币'];
assert.throws(() => validate(s1), /立场 1/);
// A31
const zh = base(); zh.families[0].name = '百叶顶组的广告'; zh.lights[0].name = '百叶顶组';

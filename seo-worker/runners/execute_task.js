...
function buildPreparePrompt(opts) {
  ...
  return [
    ...
    '-----',
    '',
    '执行器缺口的标准出口（2026-09-08 job592 教训）：如果落地需要的具体操作不在执行器白名单里',
    '（能力清单标注「执行器未实现」，或你核对后确认缺失），这不是选择题，也不需要人定：',
    '方案照常出到定稿粒度（每一处改什么写全），「API 调用序列」一节写明',
    '「执行器缺 <操作名>，标记 [capability-gap]，本单转人工按本方案逐条落地」。',
    '补不补执行器是基建排期问题，由维护者看 [capability-gap] 标记决定，不进方案的决策面。',
    '',
    ...
    '- **前提闸（2026-09-21）**：动手前先核任务书的前提与线上现状。前提被实测推翻（说缺的东西在、',
    ...
    '  在第 1 节写明抽样口径和漏检风险，apply 阶段的逐条回读兜底其余。',
    '  二是按真实数据分批：本方案只覆盖第一批，批的边界按数据划（按组、按 collection、按数量上限，不按时间划），',
    '  第 1 节末尾加「剩余批次」清单写清还剩什么和多少，由团队建后续任务下轮接着做。',
    '  拆不拆由你按撞到的真实数据量当场判断，plan 没拆不等于必须一口吃完。',
...
function lintPlan(text) {
  ...
  // 6. 选择题必须收敛（review_principles：你来选，不许原样递给人）。

...
function buildPrompt(opts) {
  ...
  return [
    ...
    '  head 层的问题你在本会话核不了：别下「站上没有 X」的结论，要写「head 层待核（工具看不到）」，',
    '  并把核验列为任务第一步交给 execute（它有带浏览器 UA 的 curl）。2026-09-21 favicon 单教训：',
    '  工具盲区被当成站点缺陷，立了一单去补本来就在的东西。',
    ...
    task ? '' : '  自动落地（失败一次熔断转人工）。**意图携带（2026-09-14 起）**：委托确认本身就是放行凭证，花钱/',
    task ? '' : '  不可逆档出方案后也直接落地，不再二次停放行卡；只有三种真障碍才停下报告：方案条目越出委托申报',
    task ? '' : '  范围、同任务已熔断、止损闩生效。执行器白名单外的 op 也照常落（agent 泳道直接执行加零模型对账），',
    task ? '' : '  别再说「执行器不支持要转人工」。对人描述流程时照这个说，别承诺「会停卡等放行」。',

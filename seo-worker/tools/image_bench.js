#!/usr/bin/env node
'use strict';
// 配图源对比：同一条 prompt 分别用平台 FLUX 1.1 Pro 和直连（默认 replicate，可 --provider bfl）各型号各出一张，图存本地目录，打印耗时。
// 灰度升级前先用博客产线真实 prompt 跑几条（从 job 日志「调 FLUX，prompt」附近拿），人眼看一遍再决定放哪几家进 canary。
//   SEO_WORKER_CONFIG=/data/aira/seo-worker/config.json REPLICATE_API_TOKEN=... \
//   node tools/image_bench.js --workspace /data/aira/clients/benscurtainsnz \
//     --models flux-1.1-pro,flux-2-pro,flux-2-klein-9b [--provider replicate|bfl] [--no-wf] [--out /tmp/bench] "<prompt>" ["<prompt 2>" ...]
...
const provider = opt('provider', 'replicate');
const VALUED = ['--workspace', '--models', '--out', '--provider'];
const prompts = [];
...
(async () => {
  ...
  for (let pi = 0; pi < prompts.length; pi += 1) {
    ...
    for (const model of models) {
      try {
        const g = provider === 'bfl' ? await imagegen.bflGenerate(cfg, prompt, { model }) : await imagegen.replicateGenerate(cfg, prompt, { model });
        const dest = path.join(out, 'p' + (pi + 1) + '-' + model + '.jpg');
        await downloadTo(g.sampleUrl, dest, { maxBytes: 8 * 1024 * 1024 });
        const usd = (g.cost || 0) / 100;
        rows.push({ prompt: pi + 1, provider, model: g.model, ms: g.ms, cost_usd: usd, predict_sec: g.predictSec || 0, file: dest });
        console.log('  ' + model.padEnd(18) + Math.round(g.ms / 1000) + 's' + (usd ? '  $' + usd.toFixed(3) : g.predictSec ? '  predict ' + g.predictSec.toFixed(1) + 's' : '') + '  ' + dest);
      } catch (e) { rows.push({ prompt: pi + 1, provider, model, error: e.message }); console.log('  ' + model.padEnd(16) + '失败 ' + e.message); }
    }
  }

 ...
 *   不给 cid 时现查 /board 取 platform=Shopify 的客户（不用手抄名单，DEFECTS 2026-09-22）。
...
async function main() {
  ...
  /* 名单现查 /board（auth_any，worker 可读，含 platform）；profile 与任务从 /context 取（auth_worker）。 */
  const board = await api.req('GET', '/board');
  const rows = (board && board.clients) || [];
  const targets = rows.filter((c) => /shopify/i.test(String(c.platform || ''))
  ...
  for (const c of targets) {
    ...
    try {
      ...
      const r = await shopifylink.syncClientLinks(api, cid, profile, (m) => console.log(stamp + ' [' + cid + ' ' + c.name + '] ' + m), { tasks: (ctx && ctx.tasks) || [] });
      total += r.changed || 0;

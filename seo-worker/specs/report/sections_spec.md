...
- **只在「下月计划」一处放表态条**（Alvin 2026-09-01 定：别的 section 不讲计划，放表态没意义），引导语固定「关于我们下个月的工作计划，您的看法是：」，两选项：同意按建议执行 / 其他反馈（带文本，空文本客户端拦、服务端 400）。没有 hold：报告不是决策卡，中间态无意义。右下角保留全局浮动「留言给我们」。按钮样式照抄 paid 方向卡（蓝色方框系 #0057b8 / #eff6ff / #bfdbfe），文案沿用方向卡用语，不要自造。**组件与提交脚本固定在模板里，不经模型**（方向卡两起脚本被改写事故的同款根治）。
...
- 提交：POST /report_feedback（公开端点，token 门），choice 取 agree / other（other 必须带文本），item 为 next 或 global_note。落 seo_report_feedback 表（全量真相），摘要追进 seo_reports.note（看板版本列表速览，超 1800 字只留表）。读取：GET /reports/{id}/feedback（auth_any）。

...
## SECTION C：电商 DTC 补充（2026-09-10，sungait 首单沉淀，决策预置防打回）

以下决策已预置，写稿与出方案时直接照办，不得作为「需要人定」上报：

1. **单品价格一律不写进正文**。价格随库存波动，写死即过期。价格表述只允许客户既有的长期口径（如 from $17.99），或不提价格让读者点商品页看现价。比较表用镜片、材质、适用场景等稳定属性列，不用价格列。
2. **榜单选品自定，标准写进方案**：在售（逐个 curl 核对 200）、覆盖至少三类款式、与主词意图匹配（如 polarized 榜单只收明文标注 polarized 的商品页）。选完即定稿。
3. **内链目标先核存在**：目标 collection/页面 404 或与主张不符（如页面无 polarized 主张却要进 polarized 榜单）即拒链并在方案里说明，不上报不等人。
4. **draft 类交付的前台预期是 404**（未发布文章无公开 URL），不是 3xx；自验断言按 404 写，不要把 404 登记为偏差。
5. 配图只用客户站 CDN 已有素材，封面选图给理由（构图、裁切适应性、无文字水印）。

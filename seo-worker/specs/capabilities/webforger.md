...
3. **redirects 只准 PATCH，严禁 PUT。** `PUT /redirects` 是全量替换，会把清单里没列出的历史重定向整片清空。任何时候都用 `PATCH` 带 `set` 和 `delete`。**跳转表不匹配带文件扩展名的路径**（`/favicon.ico`、`/sitemap.xml`、`.png` 等一律不生效，平台按静态资源直查文件系统）：给这类路径做 301 的方案在本平台做不成，别写进方案也别转人工（2026-09-21 benscurtainsnz #802/#803，两条根路径图标 301 拆给人工才发现配了也没用；根路径图标兜底本身也非必要，浏览器认 head 引用）。

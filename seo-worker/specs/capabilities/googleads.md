...
- creative-direction [risk_class: reversible]：素材方向卡。广告级成绩单、新文案确认、零曝光清理名单、测试方案。规格见 ../report/creative_direction_spec.md，验收人是 sales。客户 agree 后接 ad-copy-rewrite（external，走放行）。
- 两卡节奏与数据窗口（2026-09-15 Alvin 定）：每客户每月一张，词卡与素材卡逐月交替；数据窗口 = 发卡月前两个完整自然月，渲染器强制，细节见 ../report/keyword_direction_spec.md 数据窗口节。

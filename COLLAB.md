...
- 坑：result 端点的 ops 回填校验以 release_policy.json 为准，加新 op 记得 md/json 两处同步（specs.test 锁）；backfill 工具对全部卡型广播 [backing]（跟进任务当年已建过，再开新任务只会重复），预览必看 hold 项与不相干任务再 --apply。
- 下一步/认领：backfill --apply 等 Alvin 过目预览清单（建议剔 #620/#820）；sanmichelle GSC 403（SA 无权限）待补授权；Aiden 若在做 collections 命令面请停手认领回包。
...
- 坑：collections 写通道缺口是 SP 船队站内活的硬瓶颈（本轮 4 条里 3 条落不了地全因它），Aiden 的 shopseo collections/pages 扩容优先级应提。
- 下一步/认领：批1 第二轮验证中；对账工具（declared done vs live 现测）排上；Aiden 排 collections 扩容。
